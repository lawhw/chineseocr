...
pipeline {
    ...
    environment {
		...
		ENV_NAME = "${BRANCH_NAME == 'master' ? 'pro' : 'dev'}"
        TAG_NAME = "${BRANCH_NAME == 'master' ? 'stable' : 'test'}.${env.BUILD_NUMBER}.${GIT_TAG}"
        //TAG_NAME = 'latest'
                ...
                script {
					...
					for (int i = 0;i < prolist.size();++i){
						...
						docker.withRegistry('http://registry.apps.es1688.k8s.local', "${REGISTRY_CREDENTIALS_ID}") {
                            ...
                            escloud_data='{\\"spec\\":{\\"template\\":{\\"spec\\":{\\"containers\\":[{\\"name\\":\\"sc-'+proj_name+'\\",\\"image\\":\\"'+IMAGE_NAME+'\\"}]}}}}'
